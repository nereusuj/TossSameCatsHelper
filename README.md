...
*   **자동 플레이 (Auto Play)**: 접근성 서비스(Accessibility Service)를 이용하여 분석된 카드를 자동으로 클릭해줍니다. 한 번의 실행으로 분석과 클릭 과정을 8회 반복하여 게임을 자동으로 진행합니다.
...
1.  **앱 실행**: 앱을 실행하고 권한 요청을 허용합니다.
    ...
    *   **접근성 권한 (Accessibility Service)**: 자동 플레이 기능을 사용하기 위해 필요합니다. 앱 설정에서 해당 권한을 활성화해주세요.
2.  **게임 시작**: 토스 앱의 "같은 고양이 맞추기" 게임을 실행합니다.
3.  **자동 진행**:
    *   도우미 앱의 **'시작(Start)'** 버튼을 누릅니다.
    *   화면 분석 후 결과가 카드 위에 표시됩니다.
    *   **4초 후**, 자동으로 카드를 클릭하여 게임을 진행합니다.
    *   클릭이 완료되면 **3초 대기 후**, 자동으로 다음 분석을 시작하며 이 과정을 **8회 반복**합니다.
4.  **중지 및 종료**:
    *   진행 중에 **'중지(Stop)'** 버튼을 누르면 즉시 분석 및 자동 플레이가 중단되고 초기 화면으로 돌아옵니다.
    *   모든 반복이 완료되면 자동으로 초기 화면으로 복귀합니다.
